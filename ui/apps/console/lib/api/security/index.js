...
export function transformApiToUi(apiGroup) {
    if (!apiGroup)
        return undefined;
    const combinatorMap = {
        LOGIC_AND: 'and',
...
export function transformUiToApi(uiGroup) {
    if (!uiGroup)
        return {
            id: 'root',
    ...
    const traverse = (node) => {
        return {
            ...
            rules: (node.rules || []).map((uiRule) => {
                ...
                let fieldSource = 'FIELD_SOURCE_UNSPECIFIED';
                let key = '';
                if (uiRule.field) {
...
export function transformApiToUiRuleBased(apiData) {
    if (!apiData)
        return apiData;
...
export function transformUiToApiRuleBased(uiData) {
    if (!uiData)
        return uiData;
...
export async function listRuleBaseds(options) {
    try {
...
export async function getRuleBased(id, options) {
    try {
...
export async function createRuleBased(data, options) {
    try {
...
export async function updateRuleBased(id, data, options) {
    try {
        const endpoint = `${API_BASE_PATH}/security/rulebased/${id}`;
        const payload = transformUiToApiRuleBased(data);
