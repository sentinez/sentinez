...
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle, DialogTrigger, DialogFooter, DialogClose, } from '@sentinez/ui/components/dialog';
...
export const columns = [
    {
        ...
        cell: ({ row }) => (<div className="w-full truncate">
        <Link className="text-blue-700 font-semibold underline" href={`./rulebased/${row.original.id}`}>
          {row.getValue('name')}
        </Link>
      </div>),
    },
    ...
    },
    {
        ...
        cell: ({ row }) => {
          ...
          <BadgeStatus status={status} value={status}/>
        </div>);
        },
    },
    {
          ...
          <DialogContent className="sm:max-w-3xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="py-6 flex flex-col gap-4">
              <div className="grid gap-2">
                ...
                <Input id="name" placeholder="Rule Name" value={name} onChange={(e) => setName(e.target.value)}/>
              </div>
              <div className="grid gap-2">
              ...
              <div className="grid gap-2">
                ...
                <Input id="priority" type="number" placeholder="1" value={priority} onChange={(e) => setPriority(e.target.value)}/>
              </div>
              <div className="grid gap-2 mt-2">
                <Label>Condition Logic</Label>
                <div className="-mx-1">
                  <QueryBuilder key={open ? 'open' : 'closed'} initialQuery={query} onChange={setQuery}/>
                </div>
              </div>
            </div>
            <DialogFooter>
      ...
      <div className="w-full">
        ...
        <div className="overflow-hidden rounded-md">
          <Table className="table-fixed w-full">
            ...
            <TableBody>
              {loading ? (<TableRow className="max-h-fit">
                  <TableCell colSpan={columns.length} className="h-24 text-center">
                    Loading...
                  </TableCell>
                </TableRow>) : table.getRowModel().rows?.length ? (table.getRowModel().rows.map((row) => (<TableRow key={row.id} className="max-h-fit">
