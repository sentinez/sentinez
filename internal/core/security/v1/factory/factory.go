...
func NewDefaultHandler(ctx context.Context, appConf *confpb.Config) securitypb.SecurityServiceServer {
	service := NewDefaultService(ctx, appConf)
	return securityhdl.New(service)
}

// NewDefaultService initializes the Security service.
func NewDefaultService(ctx context.Context, appConf *confpb.Config) *securitysvc.SecurityService {
	return securitysvc.New(appConf)
}
