...
func (m *ExprLite) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *ExprLite) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *ExprLite) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if len(m.Logics) > 0 {
		for iNdEx := len(m.Logics) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.Logics[iNdEx])
			copy(dAtA[i:], m.Logics[iNdEx])
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Logics[iNdEx])))
			i--
			dAtA[i] = 0x32
		}
	}
...
func (m *MatchedRules) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.TotalScore != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.TotalScore))
		i--
		dAtA[i] = 0x20
	}
	if len(m.Scores) > 0 {
...
func (m *MatchedRules) SizeVT() (n int) {
	...
	if m.TotalScore != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.TotalScore))
	}
	n += len(m.unknownFields)
	return n
}
...
func (m *ExprLite) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: ExprLite: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ExprLite: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
